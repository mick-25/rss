...
const REGEX = {
  ...
  epRange: /((?:e)|(?:ep))?(?: )?(?<start>\d{1,4})\s?(-|~)\s?(?<end>\d{1,4})/, //xxx-xxx
  epRangewithS:
...
const rawContent = require("fs").readFileSync("./servers.txt");
...
let fetchTorrent = async (query, type = "series") => {
  ...
  let url = `${hostdata.host}/api/v2.0/indexers/all/results?apikey=${
    ...
    type == "series"
      ...
      ? "&Category%5B%5D=2000"
      : ""
  }&Category%5B%5D=8000&Tracker%5B%5D=bitsearch&cache=false`;
